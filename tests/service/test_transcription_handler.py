...
def _make_handler(use_punctuation: bool = False, config_dict: dict | None = None):
    ...
    handler = TranscriptionHandler(config, backend, audio_file_manager, ui_processor, use_punctuation)
    return handler, config, backend, audio_file_manager, ui_processor
...
class TestTranscriptionHandlerInit:
    ...
    def test_init_success(self):
        ...
        handler, config, backend, audio_file_manager, ui_processor = _make_handler(use_punctuation=True)

        assert handler.config == config
        assert handler.backend == backend
...
class TestTranscriptionHandlerTranscribeFrames:
    ...
    def test_transcribe_frames_success(self, mock_process_punct):
        """正常系: 音声フレームの文字起こし成功"""
        handler, _, backend, audio_file_manager, ui_processor = _make_handler()
        frames = [b'audio_data_1', b'audio_data_2']
        sample_rate = 16000
        audio_file_manager.save_audio.return_value = '/test/temp/audio.wav'
        backend.transcribe.return_value = "文字起こし結果"
        ...
        backend.transcribe.assert_called_once_with('/test/temp/audio.wav')
    ...
    def test_transcribe_frames_transcription_fails(self):
        """異常系: 文字起こし失敗"""
        handler, _, backend, audio_file_manager, ui_processor = _make_handler()
        audio_file_manager.save_audio.return_value = '/test/temp/audio.wav'
        backend.transcribe.return_value = None
    ...
    @patch('service.transcription_handler.process_punctuation')
    def test_transcribe_frames_cancelled_before_ui_update(self, mock_process_punct):
        """異常系: UI更新前にキャンセル"""
        handler, _, backend, audio_file_manager, ui_processor = _make_handler()
        audio_file_manager.save_audio.return_value = '/test/temp/audio.wav'
        ...
        backend.transcribe.side_effect = cancel_after_transcribe
...
class TestTranscriptionHandlerHandleAudioFile:
    ...
    def test_handle_audio_file_with_exception(self):
        """異常系: 処理中に例外発生"""
        handler, _, backend, _, _ = _make_handler()
        backend.transcribe.side_effect = Exception("処理エラー")
